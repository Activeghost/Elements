...
public class ArrayOperations
{
	...
	public static int[] getNextPermutation(int[] permutation)
	{
		...
		for(int i = lastIndex; i > 0; i--)
		{
			...
			if(nextPermutation[rightIndex] > nextPermutation[leftIndex])
			{
				...
				ArrayOperations.swap(nextPermutation, leftIndex, minIndex);
	...
	public static int[] sample(int[] inputs, int sampleSize)
	{
		...
		{
			ArrayOperations.swap(sample, i, rand.nextInt(lastIndex - i));
		}
...
class PoissonTask extends TimerTask
{
	...
	public void run()
	{
		...
		ArrayOperations.samples.add(ArrayOperations.sample(buffer, ArrayOperations.SAMPLE_SIZE));

		// schedule next occurrence based on incoming rate.
		ArrayOperations.timer.schedule(new PoissonTask(), nextPeriod(getRate()).longValue());
	}
